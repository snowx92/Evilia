...
import { useSellerSalesQuery } from '@/hooks/queries/use-seller';
...
export default function SellerSalesPage() {
  ...
  useDocumentTitle(t('seller.nav.sales'));
  const [page, setPage] = useState(1);
  const query = useSellerSalesQuery({ page, limit: DEFAULT_PAGE_SIZE });
  ...
  const exportRows = () => {
    const cols: CsvColumn<SellerSale>[] = [
      { header: 'ID', value: (s) => s.id },
      { header: 'External ID', value: (s) => s.externalId },
      { header: t('common.amount'), value: (s) => s.amount },
      { header: 'Currency', value: (s) => s.currency },
    ];
    downloadCsv(`my-sales-page-${page}`, items, cols);
  };
  ...
  const columns: Column<SellerSale>[] = [
    {
      ...
      cell: (s) => <span className="font-medium">{s.externalId}</span>,
    },
    {
      key: 'id',
      header: 'ID',
      cell: (s) => <span className="font-mono text-[11px] text-muted-foreground">{s.id}</span>,
    },
    {
      key: 'amount',
